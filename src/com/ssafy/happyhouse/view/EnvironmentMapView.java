...
public class EnvironmentMapView{
	...
	private String[]		  	title = { "번호", "동", "아파트이름", "거래금액", "거래종류" },
								envTitle = {"업체명","점검기관명","소재지주소"},
								storeTitle = {"업체명","점검기관명","소재지주소"};
...
/*		
		//환경오염지도 테이블
		envModel = new DefaultTableModel(envTitle,10);
		...
		leftBottomTop.add(new JLabel("주변 환경 지도 점검", JLabel.CENTER));
		leftBottomTop.add(envPan,"Center");
		
		//상가정보 테이블
		storeModel = new DefaultTableModel(storeTitle,10);
		storeTable = new JTable(storeModel);
		storePan = new JScrollPane(storeTable);
		storeTable.setColumnSelectionAllowed(true);
		leftBottomBottom.add(new JLabel("주변 상가 정보", JLabel.CENTER));
		leftBottomBottom.add(storePan,"Center");
		
		leftBottom.add(leftBottomTop);
//		leftBottom.add(leftBottomBottom);
*/		
